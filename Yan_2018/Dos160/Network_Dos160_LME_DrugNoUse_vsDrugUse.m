...
%WantedSubMatrix(find( ((DrugUseScore==-1)+(DrugUseScore==1)) ==0 )) = 0;

WantedSubMatrix(find( ((FirstEpisodeScore==1).*((DrugUseScore==-1))+(DrugUseScore==1)) ==0 )) = 0;


%WantedSubMatrix(find( ((FirstEpisodeScore==1).*((DrugUseScore==-1))+(FirstEpisodeScore==1).*((DrugUseScore==1))) ==0 )) = 0;
...
WantedSubMatrix=ones(length(SubID),1);
for i=1:length(SiteIndex)
    DrugUseScoreTemp=DrugUseScore(find((Site==SiteIndex(i)))); %DxTemp=Dx(find(Site==SiteIndex(i)));
    SubjectNumberPerSite(i,:)=[SiteIndex(i),length(find(DrugUseScoreTemp==1)),length(find(DrugUseScoreTemp==-1))];
    if (length(find(DrugUseScoreTemp==1))<10)||(length(find(DrugUseScoreTemp==-1))<10)
        WantedSubMatrix(find(Site==SiteIndex(i)))=0;
    end
end
...
X=[ones(size(DrugUseScore)),DrugUseScore,Age,Sex,Edu,Motion];
Z={ones(size(DrugUseScore)),DrugUseScore};
G={Site,Site};
