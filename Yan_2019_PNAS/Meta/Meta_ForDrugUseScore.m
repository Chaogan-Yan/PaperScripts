...
for i=1:length(SiteIndex)
    ...
    DrugUseScore=DrugUseScoreAll(find(Site==SiteIndex(i)));
    AllCov = [ones(length(SubID),1), DrugUseScore,Age,Sex,Edu,Motion];
    ...
    N1(i,1)=length(find(DrugUseScore==1));
    N2(i,1)=length(find(DrugUseScore==-1));
end
