function f_DFC_W_RandPhase(data_path, tpl_path, output_path, mask_path, WinLength, WinStep, WinType)
...
idx_WB = find(mean(data_all)'~=0);

dataTmp = data_all(:, idx_WB);
padLen = 2^nextpow2(tp);
dataFF = fft(dataTmp, padLen);
dataPhase = angle(dataFF);
dataPhaseRand = rand(size(dataPhase))*2*pi - pi;
dataPhaseRand([1, padLen/2+1], :) = 0;
dataPhaseRand(end:-1:padLen/2+2, :) = dataPhaseRand(2:padLen/2, :).*-1;
dataRandFF = abs(dataFF).*exp(1i*dataPhaseRand);
dataRand = ifft(dataRandFF);
dataTmp = dataRand(1:tp, :);
data_all(:, idx_WB) = dataTmp;
