...
for isub = 1:size(SubID,1)
    ...
    for icon= 1:size(mulconditions.SpeakBegin_onsetTime,1)
        jobs{1}.stats{1}.fmri_spec.sess.cond(icon).name=['SpeakThoughtOnset',num2str(icon)];
        jobs{1}.stats{1}.fmri_spec.sess.cond(icon).onset=mulconditions.SpeakBegin_onsetTime(icon);
        jobs{1}.stats{1}.fmri_spec.sess.cond(icon).duration=mulconditions.Duration(icon);
    end
    ...
    for jcon=1:size(mulconditions.SpeakBegin_onsetTime,1)
        contrastset=zeros(1,size(mulconditions.SpeakBegin_onsetTime,1));
        contrastset(jcon)=1;
        jobs{1}.stats{1}.con.consess{jcon}.tcon=struct('name',['SpeakEvent',num2str(jcon)],'convec',contrastset,'sessrep','none');
    end
