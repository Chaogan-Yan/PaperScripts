...
def vae_model(train_x,train_y,epoch,h5_fname,advh5_name,state = 'train'):
    ...
    # 设置中间表征z（16） 和 类别信息c（10）的网络参数：层数（纬度），激活函数
    DIM_Z = 32
    DIM_C = 6
    INPUT_SHAPE= 512
    ACTIVATION="tanh"
    ...
    cvae_loss = K.mean((1 + params["lambda"]) * recon_loss + params["a"]*kl_loss + params["lambda"]*kl_qzx_qz_loss + params["gamma"]*adv_loss(train_x,mean_cvae.predict([train_x,train_y]),advh5_name))
    ...
    if state == 'train':
        if not os.path.exists(h5_fname):  # 改成 path.exist()
            #x,y = RANDOM(train_x,train_y) 
        ...
        print('you ask for too much, please improve your model ASAP!!!')
